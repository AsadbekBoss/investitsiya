...
export default function LoginPage() {
  ...
  const MOCK_USERS: Record<string, { password: string; role: Role; fullName: string; id: number }> = {
    hokim:        { password: "1234", role: "HOKIM",        fullName: "Hokim Admin",        id: 1 },
    admin:        { password: "1234", role: "ADMIN",        fullName: "Administrator",       id: 2 },
    investitsiya: { password: "1234", role: "INVESTITSIYA", fullName: "Investitsiya Xodim", id: 3 },
    qurilish:     { password: "1234", role: "QURILISH",     fullName: "Qurilish Xodim",     id: 4 },
    superadmin:   { password: "1234", role: "SUPERADMIN",   fullName: "Super Admin",        id: 5 },
    user:         { password: "1234", role: "USER",         fullName: "Foydalanuvchi",      id: 6 },
    tashkilot:    { password: "1234", role: "TASHKILOT",    fullName: "Tashkilot Xodim",    id: 7 },
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (axiosErr.response?.status === 401) {
        // Server bor lekin parol noto'g'ri — mock ham tekshir
        const mock = MOCK_USERS[username.toLowerCase()];
        if (mock && mock.password === password) {
          setAuth({ id: String(mock.id), ism: mock.fullName, email: username, role: mock.role, createdAt: new Date() }, "mock-token-" + username);
          redirectByRole();
          return;
        }
        setError("Foydalanuvchi nomi yoki parol noto'g'ri");
      } else {
        // Server yo'q — mock bilan kirish
        const mock = MOCK_USERS[username.toLowerCase()];
        if (mock && mock.password === password) {
          setAuth({ id: String(mock.id), ism: mock.fullName, email: username, role: mock.role, createdAt: new Date() }, "mock-token-" + username);
          redirectByRole();
        ...
        if (mock) {
          setError("Parol noto'g'ri (offline rejim: parol — 1234)");
        } else {
          setError("Foydalanuvchi topilmadi (offline rejim)");
        }
      }
    }
    setLoading(false);
          ...
          <div className="rounded-3xl overflow-hidden"
            ...
            <div className="px-8 pt-8 pb-9">
              ...
              <div className="mt-3 px-4 py-3 rounded-xl"
                ...
                <p className="text-[10px] font-semibold mb-1" style={{ color: gold }}>⚡ Offline / Test rejim</p>
                <p className="text-[10px] leading-relaxed" style={{ color: "#8896b0" }}>
                  <span style={{ fontFamily:"monospace", background:creamDD, padding:"1px 4px", borderRadius:3 }}>hokim</span>,{" "}
                  <span style={{ fontFamily:"monospace", background:creamDD, padding:"1px 4px", borderRadius:3 }}>admin</span>,{" "}
                  <span style={{ fontFamily:"monospace", background:creamDD, padding:"1px 4px", borderRadius:3 }}>investitsiya</span>,{" "}
                  <span style={{ fontFamily:"monospace", background:creamDD, padding:"1px 4px", borderRadius:3 }}>qurilish</span>{" "}
                  — parol: <span style={{ fontFamily:"monospace", background:creamDD, padding:"1px 4px", borderRadius:3 }}>1234</span>
                </p>
              </div>
            </div>
          </div>
