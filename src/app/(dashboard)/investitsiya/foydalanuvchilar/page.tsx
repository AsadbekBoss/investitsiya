...
const ROLES = ["TADBIRKOR", "INVESTITSIYA", "TASHKILOT"] as const;
const ROLE_CFG: Record<string, { label: string; color: string; bg: string; border: string }> = {
  ...
  INVESTITSIYA: { label: "Investitsiya", color: "#1d4ed8", bg: "#eff6ff", border: "#bfdbfe" },
...
export default function InvestFoydalanuvchilarPage() {
  ...
  const filtered = useMemo(() => {
    let list = items.filter(u => ["TADBIRKOR", "INVESTITSIYA", "TASHKILOT"].includes(u.role));
    ...
    try {
      if (modal === "create") {
        ...
        const dto = form.password.trim()
          ...
          : { fullName: form.fullName, phone: form.phone, username: form.username, password: "", role: form.role, active: editing.active };
    ...
    <div className="-m-6 flex flex-col" style={{ minHeight: "calc(100vh - 60px)", background: "#faf7f0" }}>
      ...
      <div className="flex-shrink-0 px-6 pt-5 pb-4 flex items-start justify-between"
        ...
        <div>
          ...
          <p className="text-[11px] mt-0.5" style={{ color: T2 }}>Tadbirkor va investitsiya foydalanuvchilari</p>
        </div>
